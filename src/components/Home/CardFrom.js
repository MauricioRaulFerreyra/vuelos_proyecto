...
const CardFrom = ({ handleDetails, f }) => {
  ...
  const handleBuy = (aux) => {
    user && isAuthenticated
      ? history.push(aux)
      : swal(t("cardFrom.alerta"), t("cardFrom.alerta2"), "error");
    dispatch(resetData());
  };
        ...
        <div className={styles.container}>
          ...
          <div className={styles.container_departure}>
            ...
            <div style={{ display: "flex", flex: "45%" }}>
              ...
              <h4 className={styles.timeLocal}>
                {f.local_arrival.slice(11, 16)}
              </h4>
            </div>
            <div
              style={{ display: "flex", flex: "20%", justifyContent: "right" }}
            >
                  ...
                  <Popup
                    trigger={
                      <p style={{ cursor: "pointer", marginBottom: 0 }}>
                        {f.route.length > 2
                          ? f.route.length - 1 + " Stops"
                          : f.route.length - 1 + " Stop"}
                      </p>
                    }
                    position="top center"
                    on={["hover", "focus"]}
      ...
      <div className={styles.containerPriceFinal}>
        <div className={styles.price}>
          ...
          <div className={styles.containerPriceNum}>
            ...
            <h4 className={styles.padding_left} style={{ fontSize: "1.5rem" }}>
              {f.price}
            </h4>
          </div>
        </div>
        <div style={{ marginTop: "-7rem" }}>
          <h4 className={styles.taxes}>
            {t("cardFrom.imp")}: USD {flights.currency}{" "}
            <span> {(f.price * 0.8).toFixed()}</span>
          </h4>
          <h4 className={styles.finalPrice}>
            {t("cardFrom.final")}: USD{flights.currency}{" "}
            <span style={{ fontSize: "22px", color: "#2b2727" }}>
              {(f.price * 1.8).toFixed()}
            </span>
          </h4>
