...
export default function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case STOP_FILTER:
      if (action.payload.toFrom === false) {
        ...
        const filterStops =
          action.payload.value === "direct"
            ? state.flights.data.filter((p) => {
                const filterDeparture = p.route?.filter(
                  (el) => el.return === 0
                );
                const filterReturn = p.route?.filter((el) => el.return === 1);
                return (
                  filterDeparture.length === 1 && filterReturn.length === 1
                );
              })
            : state.flights.data.filter((p) => {
                const filterDeparture = p.route?.filter(
                ...
                if (action.payload.value === "1") {
                  return (
                    filterDeparture.length === 2 && filterReturn.length === 2
                  );
                } else {
                  return (
                    filterDeparture.length >= 3 && filterReturn.length >= 3
                  );
                }
              });
    ...
    case PRICE_FILTER:
      const filterPrice =
        action.payload &&
        state.flights.data.filter(
          (p) => parseInt((p.price * 1.8).toFixed()) <= parseInt(action.payload)
        );
    ...
    case DATE_FILTER:
      const filterDate =
        action.payload === "date"
          ? state.flights.data.sort((a, b) =>
              a.local_departure > b.local_departure ? 1 : -1
            )
    ...
    case AVAILABILITY_FILTER:
      const filterAvailability =
        action.payload &&
        state.flights.data.filter(
          (p) => Object.values(p.availability)[0] <= action.payload
        );
